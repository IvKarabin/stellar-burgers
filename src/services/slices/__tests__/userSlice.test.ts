import reducer, {
  setUser,
  loginUser,
  registerUser,
  getUser,
  updateUser,
  logoutUser,
  initialState
} from '../userSlice';

const mockUser = {
  email: 'test@mail.ru',
  name: 'Test'
};

describe('userSlice', () => {
  describe('setUser', () => {
    test('setUser', () => {
      const next = reducer(initialState, setUser(mockUser));
      expect(next.user).toEqual(mockUser);
      expect(next.isAuth).toBe(true);
    });

    test('setUser(null)', () => {
      const state = { ...initialState, user: mockUser, isAuth: true };
      const next = reducer(state, setUser(null));
      expect(next.user).toBeNull();
      expect(next.isAuth).toBe(false);
    });
  });

  describe('loginUser', () => {
    test('pending', () => {
      const next = reducer(
        initialState,
        loginUser.pending('', { email: '', password: '' })
      );
      expect(next.isLoading).toBe(true);
      expect(next.error).toBeNull();
    });

    test('fulfilled', () => {
      const next = reducer(
        initialState,
        loginUser.fulfilled(mockUser, '', { email: '', password: '' })
      );
      expect(next.isLoading).toBe(false);
      expect(next.user).toEqual(mockUser);
      expect(next.isAuth).toBe(true);
    });

    test('rejected', () => {
      const next = reducer(
        initialState,
        loginUser.rejected(
          new Error('fail'),
          '',
          { email: '', password: '' },
          'ERROR'
        )
      );
      expect(next.isLoading).toBe(false);
      expect(next.error).toBe('ERROR');
    });
  });

  describe('registerUser', () => {
    test('pending', () => {
      const next = reducer(
        initialState,
        registerUser.pending('', { email: '', password: '', name: '' })
      );
      expect(next.isLoading).toBe(true);
    });

    test('fulfilled', () => {
      const next = reducer(
        initialState,
        registerUser.fulfilled(mockUser, '', {
          email: '',
          password: '',
          name: ''
        })
      );
      expect(next.user).toEqual(mockUser);
      expect(next.isAuth).toBe(true);
    });

    test('rejected', () => {
      const next = reducer(
        initialState,
        registerUser.rejected(
          new Error('fail'),
          '',
          { email: '', password: '', name: '' },
          'ERROR'
        )
      );
      expect(next.isLoading).toBe(false);
      expect(next.error).toBe('ERROR');
    });
  });

  describe('getUser', () => {
    test('pending', () => {
      const next = reducer(initialState, getUser.pending(''));
      expect(next.isLoading).toBe(true);
    });

    test('fulfilled', () => {
      const next = reducer(
        initialState,
        getUser.fulfilled(mockUser, '', undefined)
      );
      expect(next.user).toEqual(mockUser);
      expect(next.isAuth).toBe(true);
    });

    test('rejected', () => {
      const next = reducer(
        initialState,
        getUser.rejected(new Error('fail'), '', undefined, 'ERROR')
      );
      expect(next.isLoading).toBe(false);
      expect(next.error).toBe('ERROR');
    });
  });

  describe('updateUser', () => {
    test('pending', () => {
      const next = reducer(initialState, updateUser.pending('', {}));
      expect(next.isLoading).toBe(true);
    });

    test('fulfilled', () => {
      const start = {
        ...initialState,
        user: { email: 'aa@aa.aa', name: 'aa' }
      };
      const next = reducer(start, updateUser.fulfilled(mockUser, '', {}));
      expect(next.user).toEqual(mockUser);
    });

    test('rejected', () => {
      const next = reducer(
        initialState,
        updateUser.rejected(new Error('fail'), '', {}, 'ERROR')
      );
      expect(next.isLoading).toBe(false);
      expect(next.error).toBe('ERROR');
    });
  });

  describe('logoutUser', () => {
    test('pending', () => {
      const next = reducer(initialState, logoutUser.pending(''));
      expect(next.isLoading).toBe(true);
    });

    test('fulfilled', () => {
      const start = { ...initialState, user: mockUser, isAuth: true };
      const next = reducer(
        start,
        logoutUser.fulfilled(undefined, '', undefined)
      );
      expect(next.user).toBeNull();
      expect(next.isAuth).toBe(false);
    });

    test('rejected', () => {
      const next = reducer(
        initialState,
        logoutUser.rejected(new Error('fail'), '', undefined, 'ERROR')
      );
      expect(next.isLoading).toBe(false);
      expect(next.error).toBe('ERROR');
    });
  });
});
